 ...
 * {@code MorphClient.clientTick} (the end of every client tick) and drawn
 * during the level renderer's submit-collection pass (on Fabric
 * {@code LevelRenderEvents.COLLECT_SUBMITS}, wired in
 * {@code MorphFabricClient}) as a world-space pass — the
...
public final class MorphAcquisitions {
    ...
    /**
     * How many effects {@link #submit} has handed to the level renderer,
     * ever (one per live effect per frame). Read by the NeoForge client test
     * driver (reflectively: it lives in another package) to prove the
     * loader's level-render hook really reaches this pass: each loader wires
     * it through its own event, and an unwired hook fails silently, with the
     * effect received and ticking but never drawn. Nothing in the mod reads
     * it. Render thread only, like every caller.
     */
    static int submitEffectCalls;
    ...
    /**
     * World-space submit pass, fed by the loader's level-render hook (Fabric:
     * {@code LevelRenderEvents.COLLECT_SUBMITS}). Takes the three values it
     * needs rather than a loader's context object, so every loader can call
     * it: the pose stack of the level renderer's submitFeatures pass, its
     * submit node collector, and the camera position from the level render
     * state.
     */
